  ...
  template: `
    @if (demo.tourActive()) {
      <div class="fixed inset-0 z-[100]">
        @if (rect(); as r) {
          <div class="absolute rounded-xl transition-all duration-200 pointer-events-none ring-2 ring-volt-400"
            [style.top.px]="r.top - 6" [style.left.px]="r.left - 6"
            [style.width.px]="r.width + 12" [style.height.px]="r.height + 12"
            style="box-shadow: 0 0 0 9999px rgba(8,8,11,.66)"></div>
        } @else {
          <div class="absolute inset-0 bg-coal-950/70"></div>
        }

        <div class="absolute w-[320px] max-w-[calc(100vw-24px)] rounded-2xl border border-coal-700 bg-coal-800 text-mist-100 shadow-2xl p-5"
          [style.top.px]="cardPos().top" [style.left.px]="cardPos().left">
          <div class="flex items-center justify-between mb-2">
            <span class="text-[11px] font-mono num text-volt-400">{{ demo.stepIndex() + 1 }} / {{ demo.steps.length }}</span>
            <button (click)="demo.endTour()" class="text-xs font-semibold text-mist-400 hover:text-mist-200 transition-colors">{{ 'demo.skip' | t }}</button>
          </div>
          <h3 class="text-base font-semibold">{{ es() ? step().title.es : step().title.en }}</h3>
          <p class="text-sm text-mist-300 mt-1.5 leading-relaxed">{{ es() ? step().body.es : step().body.en }}</p>
          <div class="flex items-center gap-2 mt-4">
            @if (demo.stepIndex() > 0) {
              <button (click)="demo.prev()" class="text-sm font-semibold text-mist-300 hover:text-white rounded-lg px-3 py-1.5 transition-colors">{{ 'demo.back' | t }}</button>
            }
            <span class="flex-1"></span>
            <button (click)="demo.next()" class="rounded-lg bg-volt-500 hover:bg-volt-400 text-white text-sm font-semibold px-4 py-1.5 transition-colors">
              {{ last() ? ('demo.finish' | t) : ('demo.next' | t) }}
            </button>
          </div>
          <div class="flex justify-center gap-1.5 mt-4">
            @for (s of demo.steps; track $index) {
              <span class="w-1.5 h-1.5 rounded-full transition-colors" [class]="$index === demo.stepIndex() ? 'bg-volt-500' : 'bg-coal-600'"></span>
            }
          </div>
        </div>
      </div>
